  ...
  window.CreateItemContent = window.createReactClass({
    ...
    _renderModelLabel(parent) {
      return (
        ...
        (parent.model.version ? ' ' + parent.model.version : '') +
        ' ' +
        parent.inventory_code
      )
    },
    ...
    _renderTechnicalDetailLine(line, index) {
      var innerHtml = line
        .replace(this._linkRegex(), "<a href='$1' target='_blank'>$1</a>")
        .replace(this._emailRegex(), "<a href='mailto:$1'>$1</a>")
      return (
        <div
    ...
    _renderAutocomplete() {
      ...
      var makeCall = (term, callback) => {
        ...
        window.leihsAjax.getAjax(
          '/manage/' +
            this.props.createItemProps.inventory_pool.id +
            '/items?paginate=true&search_term=' +
            term +
            '&not_packaged=true&packages=false&retired=false',
          {},
          (status, response) => {
            var ids = l.join(
              l.map(response, (r) => '&' + encodeURIComponent('ids[]') + '=' + r.model_id),
              ''
            )
    ...
    _renderSelectedItems() {
      return _.map(this.props.packageChildItems, (i) => {
        return (
          <div
            key={i.item.id}
            ...
            <div className="row padding-inset-xxs">
              <div className="col1of4 padding-left-s padding-top-xs">
                <strong className="font-size-m inline-block">{i.item.inventory_code}</strong>
              </div>
              <div className="col2of4 padding-top-xs">{this._renderModelName(i.model)}</div>
              <div className="col1of4 text-align-right">
                <button
                  onClick={() => this.props.onRemoveChildItem(i.item.id)}
    ...
    _renderPackageTitle() {
      ...
      return <h2 className="headline-m padding-bottom-m">{_jed('Package')}</h2>
    },
        ...
        <div className="margin-bottom-m">
          ...
          <div className="row emboss margin-vertical-xxs margin-right-xs">
            <div className="row padding-inset-xs">
              ...
              <div className="col1of2">
                <div className="row">{this._renderAutocomplete()}</div>
              </div>
            </div>
          </div>
