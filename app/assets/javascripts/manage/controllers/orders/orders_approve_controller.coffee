class window.App.OrdersApproveController extends Spine.Controller
  ...
  approve: (e)=>
    ...
    order = @order ? App.Order.findOrBuild trigger.closest("[data-id]").data()
    done = @done ? =>
      line = trigger.closest(".line")
      line.html App.Render "manage/views/orders/line_approved", order, { accessRight: App.AccessRight, currentUserRole: App.User.current.role } if line?
    fail = @fail ? (response)=>
      callback = =>
        line = trigger.closest(".line")
        new App.OrdersApproveFailedController {order: order, line: line, trigger: trigger, error: response.responseText, done: @done}
