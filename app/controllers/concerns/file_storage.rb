module FileStorage
  def store_image_with_thumbnail!(file, model)
    ActiveRecord::Base.transaction do
      ...
      thumbnail = model.images.build(content: Base64.encode64(thumbnail_file.read),
  ...
  def store_image!(file, model)
    image = model.images.build(content: Base64.encode64(file.read),
  ...
  def store_attachment!(file, **opts)
    attachment = Attachment.new(content: Base64.encode64(file.read),
                                filename: file.original_filename,
                                size: file.size,
                                content_type: file.content_type,
                                **opts)
