module TransactionHealing
  ...
  # Leihs::Middleware::Audit wraps every unsafe-method request in one outer
  ...
  # queries, e.g. a subsequent render/redirect's own callbacks) until the
  # middleware's COMMIT. Running the action in a savepoint lets `render`/
  # `redirect_to` below roll back to just that savepoint instead, healing the
  # connection while leaving the outer transaction (and deferred-constraint
  # timing) untouched. See Madek/Madek#946.
  included do
  ...
  # doesn't hit the still-aborted connection. Also heals whatever savepoint
  # is currently innermost, so it works the same whether the rescue is inside
  # or outside a call site's own `transaction(requires_new: true)` block.
  def render(...)
