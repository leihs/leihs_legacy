...
# Given "there is only a signed contract by {string} for item {string}" do | who, inventory_code |
#   step "there are no contracts"
#   step "a signed contract by '#{who}' for item '#{inventory_code}'"
# end

# copied from hand_over_steps
# When "{string} clicks on 'take_back'" do | who |
...
When "{string} chooses to take back {string}'s entry" do | manager, customer |
  @user = User.find_by_login( customer )
  get manage_take_back_path( @inventory_pool, @user )
  @reservations = assigns(:reservations)
end

# When '{string} selects all reservations and takes the items back' do | who |
...
Then "{string}'s contract should be closed" do |who|
