Feature: Assign items during hand over
  ...
  # CI-ISSUE: cannot be reproduced with live data in development mode, but with the test data in test mode yes. Seems to be some subtle bug, happening sporadically, possibly having something to do with ajax requests, rendering and timing.
   @javascript 
  Scenario: Assign an inventory code by providing an inventory code of an item and a set of selected reservations
     When I open a hand over which has multiple unassigned reservations and models in stock
      And I select a linegroup
      And I add an item which is matching the model of one of the selected unassigned reservations to the hand over by providing an inventory code
