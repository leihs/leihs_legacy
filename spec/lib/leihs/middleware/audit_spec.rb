...
describe Leihs::Middleware::Audit do
  let(:env) do
    {
      'REQUEST_METHOD' => 'POST',
      'REQUEST_PATH' => '/test',
  ...
  # Regression test for Madek/Madek#946: proves the middleware itself
  # refuses to silently succeed when the app swallows a DB-level error and
  # never heals or re-raises -- independent of any per-app controller-level
  # fix (TransactionHealing), which a raw Rack app bypasses entirely.
  it 'raises instead of silently succeeding when the app leaves the connection aborted' do
    ...
    expect { middleware.call(env) }
      .to raise_error(Leihs::Middleware::Audit::TransactionAbortedError)
  end
