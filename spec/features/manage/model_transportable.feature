Feature: Model transportable
  ...
  @manage_model_transportable
  Scenario: Transportable is visible and enabled by default for model when feature is enabled
    Given I am Mike
  ...
  @manage_model_transportable
  Scenario: Transportable is visible and enabled by default for package model when feature is enabled
    Given I am Mike
    And the current pool has alternative pickup locations enabled
    When I open the create model page
    And I mark the model as a package
    Then the transportable checkbox is visible
    And the transportable checkbox is checked
    And the transportable checkbox is enabled
    Given there is a package model
